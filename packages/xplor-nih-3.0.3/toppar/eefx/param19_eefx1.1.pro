...
bond CR   CR     450.0  1.38!  FROM B. R. GELIN THESIS AMIDE AND DIPEPTIDES
bond CR   C      450.0  1.38!  FROM B. R. GELIN THESIS AMIDE AND DIPEPTIDES
bond CR   CH1E   405.0  1.52!  EXCEPT WHERE NOTED.  CH1E,CH2E,CH3E, AND CT
bond CR   CH2E   405.0  1.52!  ALL TREATED THE SAME. UREY BRADLEY TERMS ADDED
bond CR   CH3E   405.0  1.52
bond CR   CR1E   450.0  1.38
bond CR   CT     405.0  1.53
bond CR   N      471.0  1.33
bond CR   NC2    400.0  1.33!  BOND LENGTH FROM PARMFIX9 FORCE K APROXIMATE
bond CR   NH1    471.0  1.33
bond CR   NH2    471.0  1.33
bond CR   NP     471.0  1.33
bond CR   NR     471.0  1.33
bond CR   O      580.0  1.23
bond CR   OC     580.0  1.23!  FORCE DECREASE AND LENGTH INCREASE FROM C O
bond CR   OH1    450.0  1.38!  FROM PARMFIX9 (NO VALUE IN GELIN THESIS)
bond CR   OS     292.0  1.43!  FROM DEP NORMAL MODE FIT
bond CH1E CH1E   225.0  1.53
bond CH1E CH2E   225.0  1.52
bond CH1E CH3E   225.0  1.52
bond CT   CH3E   225.0  1.52! for AIB TL
...
bond H    OH2    450.0  1.00 ! from ST2 geometry
bond LP   OH2    450.0  0.80 ! ditto
bond H    H        0.0  1.6329931 ! from ST2 geometry (for SHAKE w/PARAM)
bond HT   OT     450.0  0.9572 ! from TIPS3P geometry
bond HT   HT       0.0  1.5139 ! from TIPS3P geometry (for SHAKE w/PARAM)
...
angle CR   CR   CR      70.0 106.5!  FROM B. R. GELIN THESIS WITH HARMONIC
angle C    C    CH2E    65.0 126.5!  PART OF F TERMS INCORPORATED. ATOMS
angle CR   CR   CH2E    65.0 126.5!  PART OF F TERMS INCORPORATED. ATOMS
angle C    C    CH3E    65.0 126.5!  WITH EXTENDED H COMPENSATED FOR LACK
angle C    C    CR1E    70.0 122.5!  OF H ANGLES.
angle CR   CR   CR1E    70.0 122.5!  OF H ANGLES.
...
angle CR   CR   NH1     65.0 109.0!  TO N-METHYL ACETAMIDE VIBRATIONS.
...
angle CR1E CR   CR1E    65.0 120.5
angle CR1E C    NH1     65.0 110.5! USED ONLY IN HIS, NOT IT TRP
angle CR1E CR   NH1     65.0 110.5! USED ONLY IN HIS, NOT IT TRP
angle CR1E C    NP      65.0 122.5
angle CR1E C    NR      65.0 122.5
angle CR1E CR   NR      65.0 122.5
angle CR1E C    OH1     65.0 119.0
angle CR1E CR   OH1     65.0 119.0
...
angle NC2  CR   NC2     70.0 120.0
angle NC2  C    NH1     70.0 120.0
angle NC2  CR   NH1     70.0 120.0
...
angle OS   CH2E CH1E    70.0 125.3! TL
...
angle CH2E CH1E CH2E    70.0 109.5 !for PHL, TL
angle C    CH1E CH3E    70.0 106.5
angle C    CT   CH3E    70.0 106.5  !for AIB  TL
...
angle CR   CR1E CR1E    90.0 119.0
angle C    CR1E NH1     90.0 109.5
angle CR   CR1E NH1     90.0 109.5
angle C    CR1E NR      90.0 106.5
angle CR   CR1E NR      90.0 106.5
...
dihe CR1E CR   CR   CR1E 5.0       2     180.0! => TRP OOP. VIB 170CM 1
dihe CR1E C    C    C    2.5       2     180.0! SEE BEHLEN ET AL JCP 75:5685 81
dihe CR1E CR   CR   CR   2.5       2     180.0! SEE BEHLEN ET AL JCP 75:5685 81
dihe CR1E C    C    NH1  2.5       2     180.0
dihe CR1E CR   CR   NH1  2.5       2     180.0
dihe X    C    CH1E X    0.0       3       0.0! FROM GELIN THESIS AMIDES
dihe X    C    CH2E X    0.0       3       0.0! USING A SINGLE
dihe X    CR   CH2E X    0.0       3       0.0! USING A SINGLE
...
dihe X    CR   NC2  X    8.2       2     180.0! 1.6 FROM 1.8 TO COINCIDE WITH
dihe X    C    NH1  X    8.2       2     180.0! THE EXPERIMENTAL BARRIER.
dihe X    CR   NH1  X    8.2       2     180.0! THE EXPERIMENTAL BARRIER.
dihe X    C    NH2  X    8.2       2     180.0
dihe X    C    OH1  X    1.8       2     180.0
dihe X    CR   OH1  X    1.8       2     180.0
dihe X    C    OS   X    1.8       2     180.0 ! INFERRED FROM C-OH1
dihe X    CH2E OS   X   -0.1       3       0.0 ! from p22
...
impr CR   CR1E CR   CH2E  90.0    0   0.0! USED HERE FOR TRP CG OUT OF PLANE
...
impr CR   CR1E CR1E OH1  150.0    0   0.0! GIVE 249 CM 1 PHENOL OH OOP.
...
impr CR   X    X    CR    25.0    0   0.0! FROM BENZENE NORMAL MODE ANALYSIS
impr CR   X    X    C     25.0    0   0.0! FROM BENZENE NORMAL MODE ANALYSIS
impr C    X    X    CH2E  90.0    0   0.0! FROM TOLUENE METHYL OOP. 217 CM 1
impr CR   X    X    CH2E  90.0    0   0.0! FROM TOLUENE METHYL OOP. 217 CM 1
impr C    X    X    CH3E  90.0    0   0.0
impr C    X    X    CR1E  25.0    0   0.0
impr CR   X    X    CR1E  25.0    0   0.0
...
impr CR   X    X    NH1  100.0    0   0.0! AMIDES FIT TO N METHYL ACETAMIDE.
