...
def notrace_decorate(func):
    ...
    #wraps to get the docstring and name from the decorated function.
    #In future Python version, this sets a __wrapped__ attribute such that
    #the function signature is also reported correcly. Until then,
    #function arguments are not reported by help() or by the xplorDoc
    #facility.
    #
    ...
    def func_wrapper(*args, **kwargs):
        ...
        return #not reached
    import inspect 
    func_wrapper.__arglist__=inspect.getargspec(func)
    return func_wrapper
...
class CoverageResults:
    ...
    def update(self, other):
        ...
        for key in list(other_modules.keys()):
            if key in modules:
...
def create_results_log(results, dirname = ".", show_missing = 1,
    ...
    per_file = {}
    for filename, lineno in list(results.counts.keys()):
...
currentTrace=0
class Trace:
    def __init__(self, ignore = Ignore(), watchedModules = []):
        self.ignore = ignore
        self.ignore_names = ignore._ignore # access ignore's cache (speed hack)
        self.watchedModules = watchedModules   #modules to watch
        
        self.files = {'[stdin]': None}  # stores lines from the .py file, or None
        self.seenLineOne=[]
        global currentTrace
        currentTrace = self
    ...
    def trace(self, frame, why, arg):
        if why == 'line' and "__name__" in frame.f_globals:
            filename = frame.f_code.co_filename
            modulename = frame.f_globals["__name__"]
            ...
            ignore_it = self.ignore_names.get(modulename, -1)
            if ignore_it == -1:  # unknown filename
                #import sys #fixes a rarely seen error message
                sys.settrace(None)
                ignore_it = self.ignore.names(filename, modulename)
                sys.settrace(self.trace)
                pass

            if (len(self.watchedModules) and
                not modulename in self.watchedModules):
                return self.trace
            ...
            if filename == '<string>':
                filename = "[stdin]"
            elif (len(filename) and filename[0]!='/' and
                  not os.access(filename,os.F_OK)       ):
                filename = os.environ['XPLOR_DIR'] + '/' + filename

            if filename != '[stdin]' and filename not in files:
                files[filename] = [line.rstrip() for line in
                                   open(filename).readlines()]

            # this logic clearly fails if lineno=1 is revisited
            # but prevents lots of useless output
            if lineno==1:
                if filename in self.seenLineOne: return self.trace
                pass
            else:
                self.seenLineOne.append(filename)
                pass
            ...
            if files[filename] != None and len(files[filename])>=lineno:
                print('%s(%d): %s' % (os.path.basename(filename),
                                      lineno,
...
#
# support for suspending, resuming all tracing
#
savedTrace=None
savedCnt=0
def suspend():
    "suspend tracing until resume() called."
    global savedTrace, savedCnt
    if savedTrace:
        savedCnt += 1
        return

    savedTrace = sys.gettrace() 
    sys.settrace(None)
    return
def resume():
    "resume tracing."
    global savedTrace, savedCnt
    if sys.gettrace(): return

    if savedCnt<1:
        savedCnt=0
        sys.settrace(savedTrace)
        savedTrace=None
    else:
        savedCnt -= 1
        pass
    return
def addModuleName(name):
    "add the name (string) of a module to be traced - to the current trace"
    assert isinstance(name,str)
    try:
        current().watchedModules.append(name)
    except:
        pass
    return
def current():
    """return most recently created trace object"""
    if not currentTrace: raise Exception("trace has not been initialized")
    return currentTrace
...
def main(argv = None):
    ...
    if trace:
        t = Trace(ignore)
        try:
            run(t.trace, 'execfile(' + repr(progname) + ')')
        except IOError as err:
            _err_exit("Cannot run file %s because: %s" % \
                      (repr(sys.argv[0]), err.strerror))
