...
C local variables
     ...
     &     PHISTEPSPERRAD, PSISTEPSPERRAD, 
     &     XN, XP, XQ, YN, YP, YQ, ZN, ZP, ZQ,
     &     XMN, XNP, XPQ, YMN, YNP, YPQ, ZMN, ZNP, ZPQ,
     &     CORRECT1, CORRECT2, CORRECT3, correct4, correct5,
     &     correct6, CORRECTPHI, CORRECTPSI
      DOUBLE PRECISION XR, YR, ZR, XS, YS, ZS, XT, YT, ZT,
     &     XU, YU, ZU, RIJX, RIJY, RIJZ,
     &     RKJX, RKJY, RKJZ, RIJ, RKJ, CT, ST, THETA, THETAEQUIL,
     &     DEDTHETA, DEDTHETAEQUIL, DPRKJX, DPRKJY, DPRKJZ,
     &     DTHETAEQUILDPHI, DTHETAEQUILDPSI, RECST
      LOGICAL USEDERIV
C
C begin
C
...
C local variables
     ...
     &     XMN, XNP, XPQ, YMN, YNP, YPQ, ZMN, ZNP, ZPQ,
     &     CORRECT1, CORRECT2, CORRECT3, correct4, correct5,
     &     correct6, CORRECT7, CORRECT8, CORRECT9,
     &     CORRECTPHI, CORRECTPSI
      DOUBLE PRECISION XR, YR, ZR, XS, YS, ZS, XT, YT, ZT,
     &     XU, YU, ZU, RIJX, RIJY, RIJZ,
     &     RKJX, RKJY, RKJZ, RIJ, RKJ, CT, ST, OMEGAEQUIL,
     &     DEDOMEGA, DEDOMEGAEQUIL, DPRKJX, DPRKJY, DPRKJZ,
     &     DOMEGAEQUILDPHI, DOMEGAEQUILDPSI

      DOUBLE PRECISION XRS, XST, XTU, YRS, YST, YTU, ZRS, ZST, ZTU,
     &     AX3, AY3, AZ3, BX3, BY3, BZ3, CX3, CY3, CZ3,
     &     RAR3, RBR3, RCR3, CP3, SP3, OMEGA, CORRECTOMEGA, SWITCH3,
     &     RECSP3, RECCP3, DCPAX3, DCPAY3, DCPAZ3, DCPBX3, DCPBY3,
     &     DCPBZ3, DSPCX3, DSPCY3, DSPCZ3, DSPBX3, DSPBY3, DSPBZ3,
     &     DPRIJX3, DPRIJY3, DPRIJZ3, DPRKLX3, DPRKLY3, DPRKLZ3,
     &     DPRJKX3, DPRJKY3, DPRJKZ3
      LOGICAL USEDERIV
C
C begin
C
...
C
          DF = DEDOMEGA
          RECSP3=DF*SWITCH*SIGN(ONE,SP3)/MAX(ABS(SP3),MCONST)
          RECCP3=DF*(SWITCH+1)*SIGN(ONE,CP3)/MAX(ABS(CP3),MCONST)
C
C compute:
C  d cos( PHI )  d cos ( PHI )
C  ------------, -------------
...
C include files
      ...
      INCLUDE "numbers.fcm"
C local variables
      INTEGER COUNT, SPTR, OLDCLASS, NEWTYPE, OLDMAXANGLEDBS,
     &     OLDPHISTEPS, OLDPSISTEPS, THETYPE, CURPSIS, CURPHIS,
     &     curchis, CLASSINDEX
      DOUBLE PRECISION K1, CUTOFF, DUMMY, RENERGY, SMOOTHRATIO
      CHARACTER*4 THENAME
...
C-DOCUMENTATION-SOURCE-BEGIN
     ...
     &'  ! angle db violations greater than the specified value ',
     &'  ! for all classes or just for the named class. ',          
     &' RESEt  ! erases the angle database assignment table ',
     &' SIZE <ANGLe|DIHEdral> <int> <int>  ! Defines the ',
...
C
           ELSE IF (WD(1:4).EQ.'POTE') THEN
                ...
                IF (THENAME.EQ.'SQUA') THEN
                    WRITE(DUNIT, '(A)') 'using square well potential.'
                    ANGLEDBPOTENTIAL = SQUARE
                ELSE IF (THENAME.EQ.'HARM') THEN
                    WRITE(DUNIT, '(A)') 'using harmonic potential.'
...
C
           ELSE IF (WD(1:4).EQ.'DEBU') THEN
                TYPE *, 'NANGLEDBS = ', NANGLEDBS
                TYPE *, 'NANGLEDBCLASSES = ', NANGLEDBCLASSES
                DO COUNT = 1, NANGLEDBCLASSES
                   TYPE *, 'CLASS, NDX =', COUNT, ANGLEDBASSNDX(COUNT)
                END DO
C 
...
C i/o
      INTEGER ATOMI(*), ATOMJ(*), ATOMK(*), ATOML(*), ATOMM(*),
     &     ATOMN(*), ATOMP(*), ATOMQ(*), ATOMR(*), ATOMS(*),
     &     ATOMT(*), ATOMU(*), SEL(*), THETYPE
C local variables
...
C local variables
      DOUBLE PRECISION CURPRINT, NEXTPRINT, 
     &     RMS, VIOLS, NUMINRMS, RENERGY,
     &     CUREQUIL, CURANGLE, CURPHI, CURPSI, CURENER, CURDELTA
