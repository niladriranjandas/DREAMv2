...
# First energy file.
infile = open(glob.glob('../../beta_gamma_delta/4width_10grid/delta_marginal/*_energy.dat')[0], 'r')
...
# Second energy file.
infile = open(glob.glob('../../delta_chi/2width_10grid/delta_marginal/*_energy.dat')[0], 'r')
contents = infile.read()
infile.close()
...
# Output file.
filename = '_'.join(name1.split('_')[:2]) + '_energy.dat'
outfile = open(filename, 'w')

# Write name and atom lines.
outfile.write('name %s\n\n' % '_'.join(name1.split('_')[:2]))
