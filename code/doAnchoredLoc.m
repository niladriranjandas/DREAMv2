...
%%
 ...
 fillQ.pdbStruct = pdbread(OUTPUT.modeller_n_H);
   LOG.anchor = strcat(LOG.div_n_conquer,filesep,'anchored_loc_',protein_name,'.txt');
   LOG.prev_txt = '';
...
fillQ.x_filled = fillInPseudo_test(fillQ.X, fillQ.residue, fillQ.AtomName, fillQ.residuename,wh_Comp, A);
...
Aloc.Xref_atommap = fillQ.x_filled(:,fillQ.atom_map);
 ...
 if isempty(Aloc.gaps_resi)
    fprintf('\n=================================== NO GAPS (Stage: Anchored localization) =========================================');
...
%                                                  Aloc.wh_lo_bounds_atommap);
 ...
 %========================anchored localization============================
        ...
        %-----------------------------logs---------------------------------
         ...
         LOG.prev_txt = mexDolog(LOG.prev_txt,LOG.curr_txt,0,LOG.anchor);
        %------------------------------------------------------------------
        % tmp=cell2mat(Aloc.localize.info.fevalrec); figure;plot(1:length(tmp),tmp);hold on; title('anchored'); hold off; % 19-09-20 no fig run
 %=========================================================================
 %---------------------------write pdb ------------------------------------
 ...
 %[Aloc.localize.rama_out,fig_handler] = ramachandranMe(Aloc.file_full,'Regions',true,'Glycine',true); % 19-09-20 no fig run
 %plot_filename_rama = sprintf('%s_rama_anchor_noref_%d.fig',protein_name,OUTPUT.model_count);         % 19-09-20 no fig run
 %plot_file_rama    = strcat(local_folder_plots_postreg,filesep,plot_filename_rama);                   % 19-09-20 no fig run
 %savefig(fig_handler,plot_file_rama);                                                                 % 19-09-20 no fig run
 %LOG.curr_txt = sprintf('\n Written file %s \n',plot_file_rama);                                      % 19-09-20 no fig run
 %LOG.prev_txt = mexDolog(LOG.prev_txt,LOG.curr_txt,0,LOG.anchor);                                     % 19-09-20 no fig run
 [Aloc.localize.rama_out,~] = ramachandranMe(Aloc.file_full,'Regions',true,'Glycine',true,'Plot','None'); % 19-09-20 no fig run
...
%                                                    [1e4,1,1,-0.001],[10,10,10,10,10]);
   ...
   %[fillQ.rama_out,fig_handler] = ramachandranMe(fillQ.file_full,'Regions',true,'Glycine',true); % 19-09-20 no fig run
   %plot_filename_rama = sprintf('%s_rama_anchor_ref_%d.fig',protein_name,OUTPUT.model_count);    % 19-09-20 no fig run
   %plot_file_rama    = strcat(local_folder_plots_postreg,filesep,plot_filename_rama);            % 19-09-20 no fig run
   %savefig(fig_handler,plot_file_rama);                                                          % 19-09-20 no fig run
   %LOG.curr_txt = sprintf('\n Written file %s \n',plot_file_rama);                               % 19-09-20 no fig run
   %LOG.prev_txt = mexDolog(LOG.prev_txt,LOG.curr_txt,0,LOG.anchor);                              % 19-09-20 no fig run
   [fillQ.rama_out,~] = ramachandranMe(fillQ.file_full,'Regions',true,'Glycine',true,'Plot','None');  % 19-09-20 no fig run
   
   % figure;plot(1:length(fillQ.info.obj),fillQ.info.obj);hold on; title('refine');hold off % 19-09-20 no fig run

   %clearvars fig_handler % 19-09-20 no fig run
