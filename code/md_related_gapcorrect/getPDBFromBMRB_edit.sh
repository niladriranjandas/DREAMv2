...
#resi1="R"
if [[ "$atomname" == *"Q"* ]]; then
	anyone=`egrep -i "$resi"  pseudo.csv | grep $atomname | awk '{print $3}' | awk -F',' '{print $1}'`
	atomname=$anyone
fi
...
if [ ! -z $lines ]; then
	...
	pdb=`grep "^$resi1" $nomanclature_file | awk '{print $2}'`     # for ancloc_refine.pdb and after modeller
	#pdb=`grep "^$resi1" $nomanclature_file | awk '{print $5}'`     # for CYANA

	pdb_tmp=`echo $pdb | tr ' ' ','`

	#IFS=$' ' read -d '' -r -a pdb_atoms <<< $pdb
	IFS=$',' read -d '' -r -a pdb_atoms <<< $pdb_tmp
