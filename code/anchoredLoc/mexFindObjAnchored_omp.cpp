...
mex CXXFLAGS="\$CXXFLAGS -std=c++11 -fopenmp" CXXOPTIMFLAGS='\$CXXOPTIMFLAGS -Ofast -DNDEBUG' LDOPTIMFLAGS="$LDOPTIMFLAGS -fopenmp -O2" mexFindObjAnchored_omp.cpp vectoroperator.cpp
   ****************************************************************************
*/
...
// mexPrintf("\n grad: X0(row:%f , col:%u)\n",dim,row_X0); 
     // for the equality bound between vars
        ...
        tmp = 0;
        #pragma omp parallel for shared(tmp) private(i,d_ij,col_i,x_i,x_j,tmp_) schedule(static, n_per_thread)
        for(i=0; i < row_wex; i++){
           d_ij  = w_ex[i][2];
     ...
     // equality bound between var and anchor
        ...
        tmp = 0;
        #pragma omp parallel for shared(tmp) private(i,d_ij,col_i,x_i,a_j,tmp_) schedule(static, n_per_thread)
        for(i=0; i < row_wea; i++){
           d_ij  = w_ea[i][2];
     ...
     // upper bound between vars
        ...
        tmp = 0;
        #pragma omp parallel for shared(tmp) private(i,d_ij,col_i,x_i,x_j,dist_xij,tmp_) schedule(static, n_per_thread) 
        for(i=0; i < row_wux; i++){
           d_ij  = w_ux[i][2];
     ...
     // upper bound between vars and anchor
        ...
        tmp = 0;
        #pragma omp parallel for shared(tmp) private(i,d_ij,col_i,x_i,a_j,dist_xij,tmp_) schedule(static, n_per_thread)        
        for(i=0; i < row_wua; i++){
           d_ij  = w_ua[i][2];
     ...
     // regularization 
        ...
        tmp = 0;
        #pragma omp parallel for shared(tmp) private(i,col_i,x_i) schedule(static, n_per_thread)               
        for(i=0; i < n_var; i++){
