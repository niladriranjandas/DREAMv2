...
if [ ! -z "$found" ]; then
	...
	do
		...
		name_for_gap_=${name_for_gap::-4}	
                #echo "$name_for_gap_" "$pdbfile" "../protein/$proteinname/$seq" "$stage1data" "$uplfile" "$xplornoe" "$xplordihed"
        ...
        echo "NOT found in the listoffiles.txt"
	...
	uplfile=`xmllint --xpath "string(//upl_file)" "$paramfile"`
        #uplfile=`ls ../protein/$protname/*concatupl.upl`   #check ones
		
	# assumed that makeXplorUplAco.sh has already ran
	#xplornoe=`ls ../protein/$proteinname/*noe.tbl`
	#xplordihed=`ls ../protein/$proteinname/*aco.tbl`
        xplornoe=`xmllint --xpath "string(//upl_xplor)" "$paramfile"`
	xplordihed=`xmllint --xpath "string(//aco_xplor)" "$paramfile"`

	##### check if any xml tag is empty #####
        if [ -z "$uplfile" ]; then
		echo "FILE: uplfile not found in $paramfile"
		exit -1
	fi
        if [ -z "$xplornoe" ]; then
                echo "FILE: xplornoe not found in $paramfile"
		exit -1
        fi
	...
	while IFS= read -r line || [[ -n "$line" ]]; do
		...
		for pdbfile_i in `ls ../protein/$line/$em_folder/*/*em_nosol.pdb`			        
		do	        
	...
	while IFS= read -r line || [[ -n "$line" ]]; do
		...
		if [ ! -f ../protein/$proteinname/$uplfile ]; then
			echo "$uplfile file not found"
			exit -1
		fi
		if [ ! -f ../protein/$proteinname/$xplornoe ]; then
			echo "$xplornoe file not found"
			exit -1
		fi
		if [ ! -z "$xplordihed" ]; then
			if [ ! -f ../protein/$proteinname/$xplordihed ]; then
				echo "$xplordihed file not found"
				exit -1
			fi
	        fi
		...
		if [[ $printcount -le $MAXPRINT ]]; then 
			if [[ -z $xplordihed ]]; then
                               gap_correct/correctBadGaps.sh "$name_for_gap_" "$pdbfile_i" "../protein/$proteinname/$seq" "$stage1data" "../protein/$proteinname/$uplfile" "../protein/$proteinname/$xplornoe" &
                               #echo " "
                               #echo "gap_correct/correctBadGaps.sh $name_for_gap_ $pdbfile_i ../protein/$proteinname/$seq $stage1data ../protein/$proteinname/$uplfile ../protein/$proteinname/$xplornoe"				
			else
				gap_correct/correctBadGaps.sh "$name_for_gap_" "$pdbfile_i" "../protein/$proteinname/$seq" "$stage1data" "../protein/$proteinname/$uplfile" "../protein/$proteinname/$xplornoe" "../protein/$proteinname/$xplordihed" &
				#echo " "
				#echo "gap_correct/correctBadGaps.sh $name_for_gap_ $pdbfile_i ../protein/$proteinname/$seq $stage1data ../protein/$proteinname/$uplfile ../protein/$proteinname/$xplornoe ../protein/$proteinname/$xplordihed"
			fi
		fi		
