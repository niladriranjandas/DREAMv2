...
#def align_all_writeorgpdb(target=None,org_mol=None,mobile_selection='name ca',target_selection='name ca',cutoff=2, cycles=5, method='align'):
def align_all_writeorgpdb(target=None,org_mol=None,mobile_selection='backbone',target_selection='backbone',cutoff=2, cycles=5, method='align'):
 """
 Aligns target to all the structures
 online help: http://pldserver1.biochem.queensu.ca/~rlc/work/pymol/align_all_writeorgpdb.py
 usage: align_all_writeorgpdb target=grp_registration, org_mol=2KK1
 """
 cutoff = int(cutoff)
 cycles = int(cycles)
...
#	print "%s: %6.3f using %d atoms" % (object_name,rmsd[object_name][0],rmsd[object_name][1])
 ...
 best_aligned_pdb = '%s.pdb' %(rmsd_list[0][0])
 cmd.save(best_aligned_pdb, rmsd_list[0][0])

cmd.extend('align_all_writeorgpdb',align_all_writeorgpdb)         
