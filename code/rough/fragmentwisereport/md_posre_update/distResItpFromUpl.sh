...
###
	...
	echo "[ bonds ]"
	echo "; atom i j  func low up1 up2 fc"
	while IFS='' read -r line || [[ -n "$line" ]]; do
		...
		if [ $mapped_atm_i != 'XX' ] && [ $mapped_atm_j != 'XX' ]; then
			...
			echo $gro_atm_i $gro_atm_j
			if [ $gro_atm_i != 'XX' ] && [ $gro_atm_j != 'XX' ]; then
				flag=`awk -v dij=$d_ij -v diststrong=$diststrong -v distmedium=$distmedium -v distweak=$distweak 'BEGIN{if (dij<=diststrong) 
												print 1; 
