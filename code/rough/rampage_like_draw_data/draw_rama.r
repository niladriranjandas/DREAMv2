...
#
#added these three lines to save high quality plots (https://stackoverflow.com/questions/36314841/rstudio-error-in-plot-new-figure-margins-too-large)
#graphics.off()
# par("mar")
# par(mar=c(1,1,1,1))
#plots are save as Rplots.pdf 
tiff("Plot2.tiff", res = 600)
graphics.off()
 par("mar")
 par(mar=c(1,1,1,1))
...
grid.dir <- "top500-angles.040823/top500-angles/pct/rama/"
#scatter.filename <- "/home/niladri/Documents/Disco_etc_all_in_1/our_algo/code/rough/rampage_like_draw_data/1hmp_mmtk.tsv"
#scatter.filename <- "test.tsv"
scatter.filename <- args[2]

scatter.data <- load.scatter(scatter.filename)

#png(filename="C:/temp/ramachandran/new/1HMP.png",width=410, height=410)
#png(filename="1HMP.png",width=1000, height=1000)
png(filename=args[1],width=1000, height=1000)
...
for(rama.type in names(grid.filenames)) {
    ...
    par(mar=c(3,3,3,3), mgp=c(1.75,0.75,0), pty="s", pch=20, cex=1.2)
