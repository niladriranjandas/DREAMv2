/*
   biolib should already be installed 
sudo gcc pdbtorsions.c -I/root/include -L/root/lib -lbiop -lgen -lm -lxml2 -o pdbtorsions
change permission of pdbtorsions
*/
                    ...
                    BOOL oldStyle)
{
   ...
   /* Print title                                                       */
//   if(oldStyle)								CHANGE by NILADRI
//   {										CHANGE by NILADRI
//      fprintf(out,"               PHI      PSI     OMEGA\n");			CHANGE by NILADRI
//      fprintf(out,"--------------------------------------\n");		CHANGE by NILADRI
//   }										CHANGE by NILADRI
//   else									CHANGE by NILADRI	
//   {										CHANGE by NILADRI
                     ...
                     BOOL terse, BOOL oldStyle)
{
   ...
   if(p!=NULL)
   {
      ...
      {
//         BuildLabel(label, p, 6, FALSE);		     //----Changed by NILADRI
//         fprintf(out, "%-8s %-4s    %8.3f %8.3f %8.3f\n",  //----Changed by NILADRI
//                 label, resnam, phi, psi, omega);          //----Changed by NILADRI
         trim(resnam);
         if(strcmp(resnam, "GLY")==0)
             fprintf(out, "1HMP:chain0:%s%-d\t%f\t%f\tGlycine\n",resnam,p->resnum,phi,psi);
         else if(strcmp(resnam, "PRO")==0)
             fprintf(out, "1HMP:chain0:%s%-d\t%f\t%f\tProline\n",resnam,p->resnum,phi,psi);
         else
             fprintf(out, "1HMP:chain0:%s%-d\t%f\t%f\tGeneral\n",resnam,p->resnum,phi,psi);
