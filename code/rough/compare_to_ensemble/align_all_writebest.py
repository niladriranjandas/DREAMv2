...
#def align_all_writebest(target=None,org_mol=None,mobile_selection='name ca',target_selection='name ca',cutoff=2, cycles=5, method='align'):
def align_all_writebest(target=None,org_mol=None,mobile_selection='backbone',target_selection='backbone',cutoff=2, cycles=5, method='align'):
 """
 Aligns target to all the structures
 online help: http://pldserver1.biochem.queensu.ca/~rlc/work/pymol/align_all_writebest.py
 usage: align_all_writebest target=grp_registration, org_mol=2KK1
 """
 cutoff = int(cutoff)
 cycles = int(cycles)
...
#	print "%s: %6.3f using %d atoms" % (object_name,rmsd[object_name][0],rmsd[object_name][1])
 ...
 filename = '%s.pdb'%(rmsd_list[0][0])
 cmd.save(filename)

cmd.extend('align_all_writebest',align_all_writebest)         
