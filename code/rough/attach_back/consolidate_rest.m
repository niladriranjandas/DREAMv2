...
clearvars reg x_n_index rama %index 
 ...
 % LOG.div_conquer = strcat(LOG.div_n_conquer,filesep,'div_n_conquer.txt');
 % LOG.prev_txt = '';

index = find([localize.method]==1);
   reg.frag_biograph = biograph((test.patch_adj(index,index)>3).*(test.patch_adj(index,index)),cellstr(num2str(index(:))),'ShowWeights','on');   
   frag_tmp_ = biograph.bggui(reg.frag_biograph);
   reg.frag_fig = figure;
   copyobj(frag_tmp_.biograph.hgAxes,reg.frag_fig);
   print(reg.frag_fig,'-depsc','-r300',strcat(local_folder_plots_postreg,filesep,reg.frag_graph_name));
