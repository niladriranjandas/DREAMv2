...
#def align_all_savebest(target=None,org_mol=None,mobile_selection='name ca',target_selection='name ca',cutoff=2, cycles=5, method='align'):
def align_all_savebest(target=None,org_mol=None,save_pse=None,mobile_selection='backbone',target_selection='backbone',cutoff=2, cycles=5, method='align'):
 """
 Aligns target to all the structures
 online help: http://pldserver1.biochem.queensu.ca/~rlc/work/pymol/align_all_savebest.py
 usage: align_all_savebest target=grp_registration, org_mol=2KK1
 """
 cutoff = int(cutoff)
 cycles = int(cycles)
...
#----------------------------- 
 ...
 filename = '%s.pse' %(save_pse)
 cmd.save(filename)
#-----------------------------

cmd.extend('align_all_savebest',align_all_savebest)         
