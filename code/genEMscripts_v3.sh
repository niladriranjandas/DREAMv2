	...
	do
		...
		if [ -f $prot ]; then
			...
			protein_name=`echo $prot | awk -F'/' '{print $NF}' | awk -F'.pdb' '{print $1}'`
 			### file for preparation for md-run ###
                        ...
                        cat $mdp_folder/run_template_1.sh >> $work_folder/$c/$filename
			
			echo "    $work_folder/$c/$filename"
                        chmod 755 $work_folder/$c/$filename
			### file for md run ###
                        filename2=$protein_name"_"$c"_em_md.sh"
                        echo "#!/bin/bash" > $work_folder/$c/$filename2
                        echo "protein_name=$protein_name" >> $work_folder/$c/$filename2
                        echo "work_folder=./" >> $work_folder/$c/$filename2
                        echo "mdp_folder=./" >> $work_folder/$c/$filename2
                        echo "upl_file=$upl_file_name" >> $work_folder/$c/$filename2
                        cat $mdp_folder/run_template_2.sh >> $work_folder/$c/$filename2

                        echo "    $work_folder/$c/$filename2"
                        chmod 755 $work_folder/$c/$filename2
			#######################
