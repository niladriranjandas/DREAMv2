...
#call the scripts to correct the artifacts and fill gaps
...
if [ ! -z "$found" ]; then
	...
	do
		...
		wreffile=$name_for_gap_"_wref"
		wreffile_ext=$name_for_gap_"_wref.pdb"
		echo "grep -v TIP3 waterrefine/$name_for_gap_/$wreffile > waterrefine/$name_for_gap_/$wreffile_ext"
		echo "gap_correct/endBapResiCorrect.sh $wreffile ../protein/$proteinname/$seq waterrefine/$name_for_gap_/$wreffile_ext"
	done	
else
	...
	while IFS= read -r line || [[ -n "$line" ]]; do
		...
		if [[ $printcount -le $MAXPRINT ]]; then 
			...
			echo " "
			echo "gap_correct/correctBadGaps.sh $name_for_gap_ $pdbfile_i ../protein/$proteinname/$seq $stage1data $uplfile $xplornoe $xplordihed"
			wreffile=$name_for_gap_"_wref"
			wreffile_ext=$name_for_gap_"_wref.pdb"
			echo "grep -v TIP3 waterrefine/$wreffile > waterrefine/$wreffile_ext"
			echo "gap_correct/endBapResiCorrect.sh $wreffile ../protein/$proteinname/$seq waterrefine/$wreffile_ext"			
		fi
